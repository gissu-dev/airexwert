      ...
      <CardContent className={cn("p-5 sm:p-6", compact && "p-4 sm:p-5")}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className={cn("w-fit", compact && "self-start sm:self-auto")}
            disabled={loading}
            onClick={loadTracks}
          >
            <RefreshCw
        ...
        {!loading && tracks.length ? (
          <div
            className={cn(
              "mt-6 grid gap-3",
              compact
                ? "max-h-[380px] overflow-y-auto pr-2 sm:max-h-[440px] lg:max-h-[520px] [scrollbar-color:rgba(31,214,154,0.45)_rgba(255,255,255,0.06)] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-white/[0.04] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-primary/35 [&::-webkit-scrollbar-thumb:hover]:bg-primary/55"
                : "md:grid-cols-2"
            )}
          >
