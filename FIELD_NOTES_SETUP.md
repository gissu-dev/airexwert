...
Run this SQL in the Supabase SQL editor. It is safe for a UUID `id` column: it updates matching existing rows by slug, inserts missing rows with `gen_random_uuid()`, and deletes the discarded placeholder slugs.
...
alter table public.field_notes
  drop constraint if exists field_notes_category_check;

alter table public.field_notes
  add constraint field_notes_category_check
  check (
    category in (
      'Build Logs',
      'Projects',
...
The main goal is usefulness. I want WertWorks to be easy to update, easy to understand, and flexible enough to grow with whatever I build next.$$,
    ...
    '3 min',
    '2026-07-02T12:00:00.000Z'::timestamptz,
    null::text,
...
targets as (
  select
    ...
    (
      ...
      where f.slug in (n.slug, n.old_slug)
      order by case when f.slug = n.slug then 0 else 1 end
      limit 1
    ) as existing_id
  from notes n
),
updated as (
  update public.field_notes f
  set
    title = t.title,
    slug = t.slug,
    category = t.category,
